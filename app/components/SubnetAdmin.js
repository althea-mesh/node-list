...
class SubnetAdmin extends Component {
  ...
  generateIp = () => {
    ...
    console.log("FUCK", ipAddress.toString());
  ...
  checkNode = () => {
    ...
    if (nodes)
      checkResult =
        nodes.findIndex(
          n => n.ethAddress.toLowerCase() === checkAddress.toLowerCase()
        ) > -1;
  ...
  removeNode = async () => {
    ...
    if (nodes)
      node = nodes.find(
        n => n.ethAddress.toLowerCase() === removeAddress.toLowerCase()
      );
  ...
  ipExists = ip => {
    let { nodes } = this.props;
    if (nodes) return nodes.findIndex(n => n.ipAddress === this.hexIp(ip)) > -1;
  ...
  addNode = async () => {
    ...
    try {
      let res = await new Promise((resolve, reject) => {
        this.props.app
          .addMember(ethAddress, ipAddress, nickname)
          .subscribe(resolve, reject);
      });
    } catch (e) {
      console.log(e);
  ...
  formatIp = async e => {
    ...
    if (addr.isValid())
      this.setState({ ipAddress: addr.correctForm() + addr.subnet });
  };
        ...
        <Row>
          <Col md={6}>
            <StyledCard>
              ...
              <Field label={t("ipAddress")}>
                <Row>
                  ...
                  <Col md={4} style={{ textAlign: "center" }}>
                    <Button mode="outline" onClick={this.generateIp}>
                      Generate IP
                    </Button>
                  </Col>
                </Row>
              ...
              <Field label={t("ethAddress")}>
                <Row>
                  ...
                  <Col md={4} style={{ textAlign: "center" }}>
                    <React.Fragment>
                      {scanning || (
                        <Button mode="outline" onClick={this.startScanning}>
                          Scan QR
                        </Button>
                      )}
                      {scanning && (
                        <Button
                          mode="outline"
                          onClick={() => this.setState({ scanning: false })}
                        >
                          Stop Scanning
                        </Button>
                      )}
              ...
              <Field>
                <Button onClick={this.addNode} mode="outline">
                  {t("addNode")}
                </Button>
              </Field>
            </StyledCard>
            <StyledCard>
              ...
              <p>{t("youHave", { bills })}</p>
              <Button onClick={this.collectBills} mode="outline">
                {t("collectBills")}
              </Button>
            </StyledCard>
          </Col>
          <Col md={6}>
            <StyledCard>
              ...
              {checkResult !== null && !checkResult && (
                <Info title="The node could not be found" />
              )}
              <Field label={t("ethAddress")}>
                <TextInput
                  wide
              ...
              <Button onClick={this.checkNode} mode="outline">
                {t("checkNodeInSubnetDAO")}
              </Button>
            </StyledCard>
            <StyledCard>
              <Text size="xlarge">{t("removeNode")}</Text>
              {removeResult && <Info title="Node successfully removed" />}
              {removeResult !== null && !removeResult && (
                <Info title="Can't find node" />
              )}
              <Field label={t("ethAddress")}>
                <TextInput
                  wide
              ...
              <Button onClick={this.removeNode} mode="outline">
                {t("removeNodeFromSubnetDAO")}
              </Button>
            </StyledCard>
            <StyledCard>
              ...
              {perBlockFeeResult && (
                <Info title="Per block fee succesfully updated" />
              )}
              {perBlockFeeResult !== null && !perBlockFeeResult && (
                <Info title="Can't find node" />
              )}
              <Field>
                <TextInput
                  ...
                  onChange={e => {
                    this.setState({ newPerBlockFee: e.target.value });
                  }}
                  value={this.state.newPerBlockFee}
                />
              </Field>
              <Button onClick={this.setPerBlockFee} mode="outline">
                {t("updatePerBlockFee")}
              </Button>
            </StyledCard>
          </Col>
